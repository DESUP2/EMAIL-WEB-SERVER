...
- Disable the default spellchecker option using spell.email.net (#8182)
...
- Removed some deprecated methods: https://github.com/email/emailmail/commit/454b0b1c
- Remove backward compatibility "layer" of bc.php (#4902)
...
- Added config option 'imap_log_session' to enable email <-> IMAP session ID logging
...
- email Framework:
    ...
    Framework files moved to lib/email
...
- Make email render the Email Standards Project Acid Test correctly
...
- Allow skins to define/override texts with <email:label />
...
- Fix possible messages exposure when using email behind a proxy (#2516)
...
- Fix email hangs on empty inbox with bincimapd (#2375)
